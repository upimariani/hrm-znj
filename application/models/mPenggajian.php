...
class mPenggajian extends CI_Model
{
	public function select($id_karyawan, $month)
	{
		...
		$data['absensi_hadir'] = $this->db->query("SELECT COUNT(karyawan.id_karyawan) as jml, karyawan.id_karyawan, stat_absensi FROM absensi JOIN karyawan ON karyawan.id_karyawan=absensi.id_karyawan JOIN karyawan_daftar ON karyawan_daftar.id_kar_daftar=karyawan.id_kar_daftar WHERE stat_absensi='1' AND approved='1' AND karyawan.id_karyawan='" . $id_karyawan . "' AND month(tgl_absensi) = '" . $month . "' AND absensi.id_penggajian = '0';")->row();
	...
	//penggajian karyawan
	public function gaji_karyawan($id_karyawan)
	{
		return $this->db->query("SELECT * FROM `penggajian` JOIN absensi ON absensi.id_penggajian = penggajian.id_penggajian WHERE id_karyawan='" . $id_karyawan . "' GROUP BY month(tgl_absensi);")->result();
	}
	public function konfirmasi_karyawan($id_penggajian, $data)
	{
		$this->db->where('id_penggajian', $id_penggajian);
		$this->db->update('penggajian', $data);
	}
